...
Requesting `per_page=150` returns a 400 with `error.code: validation_error` and a message naming the offending field (`field 'query -> per_page': Input should be less than or equal to 100`) — the cap is enforced, not silently clamped. Check the [API Reference](/api-reference) for any endpoint-specific overrides (most use the same defaults).
...
Use `has_more` to drive your iteration loop on search results, not `total_pages` (semantic search returns `total: null` because the scoring model doesn't compute a count):
